...
followerBig::followerBig(){
  ...
  //radius = 10;
    ...
    petalRepelled = 7;///number of petals that use repulsion
...
//--------------------------------------------------------------
void followerBig::setInitialCondition(ofImage &Daisy, ofImage &Petal, ofImage &ImageHovered, float px, float py, float vx, float vy, bool isLoveme){
    ...
    preTimer = ofGetElapsedTimeMillis();
...
void followerBig::update(){
    ...
    for(int i=0; i<4; i++){
        ...
        if(bPetalFixed && timer-preTimer > 10000){//timer amount for beginning prohibitive interaction
            //if (dis<136 ) {
                     if (dis<136 ) {
